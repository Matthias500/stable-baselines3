        ...
        for gradient_step in range(gradient_steps):
            for task in range(self.nr_tasks):
                ...
                if self.ent_coef_optimizer is not None:
                    ...
                    if task == 0:
                        sum_ent_coef_loss = ent_coef_loss
                    else:
                ...
                with th.no_grad():
                    ...
                    if self.use_pearl:
                        next_actions, next_log_prob = self.actor.action_log_prob(pearl_next_observations)
                        next_q_values = th.cat(self.critic_target(pearl_next_observations, next_actions), dim=1)
                    else:
                ...
                if self.use_pearl:
                    current_q_values = self.critic(pearl_observations, replay_data.actions)
                else:
                ...
                if task == 0:
                    ...
                    sum_critic_loss.add(critic_loss)

                # Compute actor loss
                # Alternative: actor_loss = th.mean(log_prob - qf1_pi)
                # Mean over all critic networks
                if self.use_pearl:
                    q_values_pi = th.cat(self.critic(pearl_observations_detached, actions_pi), dim=1)
                else:
                ...
                if task == 0:
                    ...
                    sum_actor_loss.add(actor_loss)

            #Optimize over sum of losses per task
            # Optimize entropy coefficient, also called
            # entropy temperature or alpha in the paper
            if ent_coef_loss is not None:
                self.ent_coef_optimizer.zero_grad()
                sum_ent_coef_loss.backward()
